...
export default function TransportUsagePage() {
  ...
  const { data: usage = [], isFetching } = useQuery({
    queryKey: ["transport-usage", dateFrom, dateTo],
    queryFn: () => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Транспорт — занятость</h1>
        <p className="text-slate-400 text-sm mt-1 max-w-3xl leading-snug">
          Показывает мероприятия (сплавы), где назначен транспорт. Отменённые сплавы не учитываются.
        </p>
      </div>
      ...
      <div className="space-y-4">
          ...
          <div key={row.vehicle.id} className="rounded-xl border border-slate-700 overflow-hidden">
            <div className="px-4 py-3 bg-slate-800/50 border-b border-slate-700">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {row.vehicle.organization?.trim() || "Без организации"}
        ...
        {usage.length === 0 && <p className="text-slate-500 text-sm">Транспорта пока нет или нет событий.</p>}
      </div>
    </div>
  );
}
