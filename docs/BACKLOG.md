...
- [ ] `m6-payments-core`: Оплаты - платежи, частичные оплаты, возвраты, авто-статусы оплаты. (сделано: возврат платежа из UI заказа + отображение статусов + пересчёт `paid_amount/payment_status` от суммы CONFIRMED платежей (устойчиво к возвратам/вебхукам) + аудит confirm/refund)
